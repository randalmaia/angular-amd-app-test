...
(function(global) {
			...
			callPlugin : function() {
				...
				on(
						...
						bind(
								this,
								function(plugin) {
									var load, normalizedMap, normalizedMod, bundleId = getOwn( bundlesMap, this.map.id ), name = this.map.name, parentName = this.map.parentMap ? this.map.parentMap.name
											: null, localRequire = context.makeRequire( map.parentMap, {
										enableBuildCallback : true
									} );
									...
									load.fromText = bind(
											...
											function(text, textAlt) {
												...
												try {
													...
													return onError( makeError( 'fromtexteval', 'fromText eval for '
															+ id + ' failed: ' + e, e, [ id ] ) );
												}
			...
			enable : function() {
				...
				each( this.depMaps, bind( this, function(depMap, i) {
					...
					if (typeof depMap === 'string') {
						...
						depMap = makeModuleMap( depMap, (this.map.isDefine ? this.map : this.map.parentMap), false,
								!this.skipMap );
				...
				mixin( localRequire,
						{
							...
							toUrl : function(moduleNamePlusExt) {
								var ext, index = moduleNamePlusExt.lastIndexOf( '.' ), segment = moduleNamePlusExt
										.split( '/' )[0], isRelative = segment === '.' || segment === '..';
								...
								return context.nameToUrl( normalize( moduleNamePlusExt, relMap && relMap.id, true ),
										ext, true );
							},
	...
	req.createNode = function(config, moduleName, url) {
		var node = config.xhtml ? document.createElementNS( 'http://www.w3.org/1999/xhtml', 'html:script' ) : document
