...
define( function() {
	var orig_angular, alt_angular, alternate_queue = [], app_name, app_injector, app_cached_providers = {};
	...
	/**
	 * Create an alternate angular so that subsequent call to angular.module
	 * will queue up the module created for later processing via the
	 * .processQueue method.
	 * 
	 * This delaying processing is needed as angular does not recognize any
	 * newly created module after angular.bootstrap has ran. The only way to add
	 * new objects to angular post bootstrap is using cached provider.
	 * 
	 * Once the modules has been queued, processQueue would then use each
	 * module's _invokeQueue and _runBlock to recreate object using cached
	 * $provider. In essence, creating a duplicate object into the current
	 * ng-app. As result, if there are subsequent call to retrieve the module
	 * post processQueue, it would retrieve a module that is not integrated into
	 * the ng-app.
	 * 
	 * Therefore, any subsequent angular.module call to retrieve the module
	 * created with alternate angular will return undefined.
	 * 
	 */
	...
	/**
	 * Helper function to generate angular's $routeProvider.route. 'config'
	 * input param must be an object.
	 * 
	 * Populate the resolve attribute using either 'controllerUrl' or
	 * 'controller'. If 'controllerUrl' is passed, it will attempt to load the
	 * Url using requirejs and remove the attribute from the config object.
	 * Otherwise, it will attempt to populate resolve by loading what's been
	 * passed in 'controller'. If neither is passed, resolve is not populated.
	 * 
	 * This function works as a pass-through, meaning what ever is passed in as
	 * 'config' will be returned, except for 'controllerUrl' attribute.
	 * 
	 */
	angularAMD.prototype.route = function(config) {
		...
		/*
		 * If controllerUrl is provided, load the provided Url using requirejs.
		 * Otherwise, attempt to load the controller using the controller name.
		 * In the later case, controller name is expected to be defined as one
		 * of 'paths' in main.js.
		 */
	...
	/**
	 * Recreate the modules created by alternate angular in ng-app using cached
	 * $provider. As AMD loader does not guarantee the order of dependency in a
	 * require([...],...) clause, user must make sure that dependecies are
	 * clearly setup in shim in order for this to work.
	 * 
	 * HACK ALERT: This method relay on inner working of angular.module code,
	 * and access _invokeQueue and _runBlock private variable. Must test
	 * carefully with each release of angular.
	 */
	angularAMD.prototype.processQueue = function() {
		...
		while (alternate_queue.length) {
			var item = alternate_queue.shift(), invokeQueue = item.module._invokeQueue, y;

			// Setup the providers define in the module
			for ( y = 0; y < invokeQueue.length; y += 1) {
				var q = invokeQueue[y], provider = q[0], method = q[1], args = q[2];

				if (app_cached_providers.hasOwnProperty( provider )) {
					var cachedProvider = app_cached_providers[provider];
					// console.log("'" + item.name + "': applying " + provider +
					// "." + method + " for args: ", args);
			...
			if (item.module._runBlocks) {
				angular.forEach( item.module._runBlocks, function processRunBlock(block) {
					// console.log("'" + item.name + "': executing run block: ",
					// run_block);
	...
	/**
	 * Create inject function that uses cached $injector. Designed primarly to
	...
	/**
	 * Initialization of angularAMD that bootstraps AngularJS. The objective is
	 * to cache the $provider and $injector from the app to be used later.
	 * 
	 * enable_ngload:
	 */
	angularAMD.prototype.bootstrap = function(app, enable_ngload, elem) {
		...
		// Store reference to original angular which also used to check if
		// bootstrap has take place.
